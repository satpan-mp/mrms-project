...
| TD-017 | Enterprise HTTP hardening (helmet, `@nestjs/throttler`, compression, body-size limit) | Deferred during the certification freeze | Installed + wired in `feature/final-production-readiness` (helmet, compression, global ThrottlerGuard, 1 MB body limits, process guards). Log redaction remains (see TD-020) | M | ✅ Resolved (final-production-readiness) |
| TD-020 | Pino sensitive-data redaction + Prisma query logging | Observability/security hardening | Redaction was already present in `LoggerModule` (authorization/cookie/x-device-token/set-cookie); Prisma `log` config added to `PrismaService` (query in dev, warn+error always) | L | ✅ Resolved (final-engineering-review) |
| TD-021 | ThrottlerGuard uses in-memory storage | Fine for single instance | Switch to Redis storage (`@nest-lab/throttler-storage-redis` or ioredis) for multi-instance rate limiting | L | Open |
